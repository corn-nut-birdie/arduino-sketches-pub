...
- 4 wheel 2 layer small car chassis kits, with 4 TT motors that operate at 3~6V. Soldered the 2 motors on the left side together, as well as the 2 on the right side together. Make sure that each two motors on the same side turn in the same direction, powering by the same DC current.
